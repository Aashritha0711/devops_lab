...
def aes_encrypt_bytes(pt, key):
    keys = key_expansion(list(key))
    pt = pad(pt)
    out = b''
    for i in range(0,len(pt),16):
        out += encrypt_block(pt[i:i+16], keys)
    return out

def aes_decrypt_bytes(ct, key):
    keys = key_expansion(list(key))
    out = b''
    for i in range(0,len(ct),16):
        out += decrypt_block(ct[i:i+16], keys)
    return unpad(out)

key = bytes.fromhex("000102030405060708090a0b0c0d0e0f")
pt  = bytes.fromhex("00112233445566778899aabbccddeeff")
ct  = aes_encrypt_bytes(pt, key)
print(ct.hex())
print(aes_decrypt_bytes(ct, key).hex())
